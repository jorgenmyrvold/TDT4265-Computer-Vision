...
if __name__ == "__main__":
    ...
    # w0 = model.w[:-1].reshape((28, 28, 10))
    # w1 = model1.w[:-1].reshape((28, 28, 10))
    #
    # fig, axs = plt.subplots(2, 10)
    # for i in range(10):
    #     img0 = w0[:, :, i].astype(float) / 255
    #     img1 = w1[:, :, i].astype(float) / 255
    #     axs[0, i].axis('off')
    #     axs[1, i].axis('off')
    #     axs[0, i].imshow(img0)
    #     axs[1, i].imshow(img1)
    ...
    for i in range(4):
        ...
        utils.plot_loss(val_history["accuracy"], f"lambda={l2_lambdas[i]}")
        plt.xlabel("Number of Training Steps")
        plt.ylabel("Accuracy")
        plt.legend()
